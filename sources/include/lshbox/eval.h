...
class Stat
{
    ...
    float max;
public:
    Stat(): count(0), sum(0), square_sum(0), min(std::numeric_limits<float>::max()), max(-std::numeric_limits<float>::max()) {}
    ~Stat() {}
    void reset()
    {
        count = 0;
    ...
    float getStd() const
    {
        if (count > 1)
        {
            return std::sqrt((square_sum - (sum / count) * sum) / (count - 1));
        }
        else
        {
            return 0;
